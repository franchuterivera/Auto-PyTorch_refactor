...
class SimpleImputer(BaseImputer):
    ...
    def fit(self, X: Dict[str, Any], y: Any = None) -> BaseImputer:
        ...
        if len(X["categorical_columns"]):
            if self.categorical_strategy == "constant_!missing!":
                self.preprocessor["categorical"] = SklearnSimpleImputer(
                    strategy="constant", fill_value="!missing!", copy=False
                )
            else:
                self.preprocessor["categorical"] = SklearnSimpleImputer(strategy=self.categorical_strategy, copy=False)
        if len(X["numerical_columns"]):
            if self.numerical_strategy == "constant_zero":
                self.preprocessor["numerical"] = SklearnSimpleImputer(strategy="constant", fill_value=0, copy=False)
            else:
    ...
    def get_hyperparameter_search_space(dataset_properties: Optional[Dict[str, Any]] = None) -> ConfigurationSpace:
        ...
        assert dataset_properties is not None, (
            "To create hyperparameter search space" ", dataset_properties should not be None"
        )
        if len(dataset_properties["numerical_columns"]):
            numerical_strategy = CategoricalHyperparameter(
                "numerical_strategy", ["mean", "median", "most_frequent", "constant_zero"], default_value="mean"
            )
            cs.add_hyperparameter(numerical_strategy)

        if len(dataset_properties["categorical_columns"]):
            categorical_strategy = CategoricalHyperparameter(
                "categorical_strategy", ["most_frequent", "constant_!missing!"], default_value="most_frequent"
            )
