...
class MLPNet(BaseNetworkComponent):
    ...
    def build_network(self, in_features: int, out_features: int) -> torch.nn.Module:
        ...
        layers.append(torch.nn.Linear(self.config["num_units_%d" % self.config["num_groups"]], out_features))
        network = torch.nn.Sequential(*layers)
        return network

    def _add_layer(self, layers: List[torch.nn.Module], in_features: int, out_features: int, layer_id: int) -> None:
        """
    ...
    def get_hyperparameter_search_space(
        ...
        num_groups = UniformIntegerHyperparameter("num_groups", min_mlp_layers, max_mlp_layers, default_value=5)
        ...
        for i in range(1, max_mlp_layers + 1):
            n_units_hp = UniformIntegerHyperparameter(
                "num_units_%d" % i, lower=min_num_units, upper=max_num_units, default_value=20
            )
            ...
            if dropout:
                dropout_hp = UniformFloatHyperparameter("dropout_%d" % i, lower=0.0, upper=0.8, default_value=0.5)
