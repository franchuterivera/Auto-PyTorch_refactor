...
class SearchSpace:
    ...
    def __init__(
        self, cs_name: str = "Default Hyperparameter Config", seed: int = 11,
    ):
        ...
        Returns:
        """
        self._hp_search_space = cs.ConfigurationSpace(name=cs_name, seed=seed,)

    @typing.no_type_check
    def add_hyperparameter(
        self, name: str, hyperparameter_type: str, **kwargs,
    ):
        ...
        """
        missing_arg = SearchSpace._assert_necessary_arguments_given(hyperparameter_type, **kwargs,)

        if missing_arg is not None:
            raise TypeError(f"A {hyperparameter_type} must have a value for {missing_arg}")
        else:
            hyperparameter = SearchSpace.hyperparameter_types[hyperparameter_type](name=name, **kwargs,)
            self._hp_search_space.add_hyperparameter(hyperparameter)

        return hyperparameter

    @staticmethod
    @typing.no_type_check
    def _assert_necessary_arguments_given(hyperparameter_type: str, **kwargs,) -> Optional[str]:
    ...
    def set_parent_hyperperparameter(
        self, child_hp, parent_hp, parent_value,
    ):
        ...
        Returns:
        """
        self._hp_search_space.add_condition(cs.EqualsCondition(child_hp, parent_hp, parent_value,))

    @typing.no_type_check
    def add_configspace_condition(
        self, child_hp, parent_hp, configspace_condition, value,
    ):
        ...
        Returns:
        """
        self._hp_search_space.add_condition(configspace_condition(child_hp, parent_hp, value,))
