...
class TestNormalizerChoice(unittest.TestCase):
    def test_get_set_config_space(self):
        ...
        self.assertListEqual(
            sorted(cs.get_hyperparameter("__choice__").choices), sorted(list(normalizer_choice.get_components().keys()))
        )
        ...
        for i in range(5):
            ...
            self.assertEqual(
                normalizer_choice.choice.__class__, normalizer_choice.get_components()[config_dict["__choice__"]]
            )
