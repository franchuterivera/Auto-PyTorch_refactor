...
class TestRescalerChoice(unittest.TestCase):
    def test_get_set_config_space(self):
        ...
        self.assertListEqual(
            sorted(cs.get_hyperparameter("__choice__").choices), sorted(list(rescaler_choice.get_components().keys()))
        )
        ...
        for i in range(5):
            ...
            self.assertEqual(
                rescaler_choice.choice.__class__, rescaler_choice.get_components()[config_dict["__choice__"]]
            )
    ...
    def test_only_categorical(self):
        ...
        configspace = (
            chooser.get_hyperparameter_search_space(dataset_properties).sample_configuration().get_dictionary()
        )
